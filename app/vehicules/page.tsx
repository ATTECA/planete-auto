...
const getDetail = (vehicle: (typeof vehicles)[number], label: string) => vehicle.details.find(([key]) => key === label)?.[1] ?? ''
const prices = vehicles.map((vehicle) => Number(vehicle.price.replace(/\D/g, '')))
const years = vehicles.map((vehicle) => Number(vehicle.year))
const mileages = vehicles.map((vehicle) => Number(vehicle.km.replace(/\D/g, '')))
const filterOptions = {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return [...result].sort((a, b) => sort === 'Prix croissant'
      ...
      : sort === 'Prix décroissant'
        ...
        : 0)
  }, [brand, color, fuel, gearbox, maxMileage, maxPrice, model, query, sort, yearFrom, yearTo])
