...
import { ArrowLeft, ArrowUpRight, Armchair, Calendar, ClipboardList, Clock3, Fuel, Gauge, Mail, Phone, Radio, Settings2, ShieldCheck, SunMedium } from 'lucide-react'
import Link from 'next/link'
import { getVehicle, parseVehicleIdParam, vehicleSlug, vehicleStatusBadgeClassName } from '@/lib/vehicles'
...
export default async function VehicleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const facts = vehicle.details ?? []
  const factsHalf = Math.ceil(facts.length / 2)
  const factsLeft = facts.slice(0, factsHalf)
  const factsRight = facts.slice(factsHalf)
  ...
  return <main className="vehicle-detail-page"><SiteHeader />
    <div className="vehicle-detail-shell">
      ...
      <div className="vehicle-detail-layout">
        ...
        <aside className="vehicle-aside">
          <div className="vehicle-purchase">
            <div className="vehicle-purchase-top"><span className={`vehicle-status ${vehicleStatusBadgeClassName(vehicle.status)}`}>{vehicle.status}</span><div className="vehicle-purchase-top-right"><span className="vehicle-ref">Réf. {reference}</span><VehicleShareButton title={`${vehicle.name} — ${vehicle.price}`} /></div></div>
            <h1>{vehicle.name}</h1>
            ...
            <div className="showroom-actions">
              ...
              <a className="button button-black showroom-cta" href="#offre">Faire une offre ou demander un essai <ArrowUpRight /></a>
            </div>
      ...
      <div className="vehicle-body-with-finance">
        <div className="vehicle-body-main">
          <section className="vehicle-facts">
            ...
            <div className="facts-flat-columns">
              <div className="facts-grid">{factsLeft.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
              <div className="facts-grid">{factsRight.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
            </div>
          </section>
        ...
        <div className="vehicle-body-finance">
